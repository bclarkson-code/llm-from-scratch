...
// Wrapper for forward attention

static PyObject *py_attention_forward(PyObject *self, PyObject *args) {
  PyObject *output, *stats, *input;
  long B, T, NH, C;
  PyObject *stream_obj;

  // check that the passed arguments are the right type
  if (!PyArg_ParseTuple(args, "OOOllllO", &output, &stats, &input, &B, &T, &NH,
                        &C, &stream_obj)) {
    PyErr_Print();
    return NULL;
  }

  LOG("Parsed arguments in py_attention_forward");
  LOG("B: " << B << ", T: " << T << ", NH: " << NH << ", C: " << C
            << ", out_shape: " << out_shape);

  floatX *output_ptr, *input_ptr;
  float *stats_ptr;
  ...
  stats_ptr = static_cast<float *>(get_array_pointer(stats, "stats"));
  if (!stats_ptr)
    return NULL;
  ...
  try {
    attention_forward_cudnn(output_ptr, stats_ptr, input_ptr, B, T, NH, C,
                            cuda_stream);
  ...
  LOG("Completed py_attention_forward");

  Py_RETURN_NONE;
}

// // Wrapper for  weight gradient calculation
// static PyObject *py_attention_backward(PyObject *self, PyObject *args) {
...
//
//   try {
//     attention_backward_cudnn(output_ptr, input_ptr, grad_ptr, B, T, C,
//                              out_shape, cuda_stream);
//   } catch (const std::exception &e) {
//     PyErr_SetString(PyExc_RuntimeError, e.what());
//     return NULL;
//   }
