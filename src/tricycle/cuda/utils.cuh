/*
 ...
 In particular, we define a series of macros xxxCheck that call the
 corresponding C standard library function and check its return code. If an
...
extern inline FILE *fopen_check(const char *path, const char *mode,
  ...
  if (fp == NULL) {
    ...
    fprintf(stderr,
            "---> HINT 1: dataset files/code have moved to dev/data recently "
            "(May 20, 2024). You may have to mv them from the legacy data/ dir "
            "to dev/data/(dataset), or re-run the data preprocessing script. "
            "Refer back to the main README\n");
    fprintf(stderr,
...
extern inline void fseek_check(FILE *fp, long off, int whence, const char *file,
                               int line) {
...
extern inline void fwrite_check(void *ptr, size_t size, size_t nmemb,
                                FILE *stream, const char *file, int line) {
  ...
  if (result != nmemb) {
    if (feof(stream)) {
      ...
      fprintf(
          stderr,
          "Error: Partial write at %s:%d. Expected %zu elements, wrote %zu\n",
          file, line, nmemb, result);
    }
    fprintf(stderr, "Error details:\n");
    fprintf(stderr, "  File: %s\n", file);
...
extern inline void token_check(const int *tokens, int token_count,
                               int vocab_size, const char *file, int line) {
